...
/**
 * 认证入口（{@code /api/auth/*}）——建 SSO 会话 + 发 code（CONTEXT 接口命名空间 / issue #15）。
 *
 * <p>密码登录：验凭据 → 建 SSO 会话 + 种 cookie → 发 code → 302 回 {@code redirect_uri?code&state}。
 * 登录失败抛领域异常，留登录页显示（不回业务应用）。</p>
 */
@RestController
...
public class AuthController {
    ...
    public AuthController(SsoLoginAppService loginService, SsoCookieService cookieService) {
