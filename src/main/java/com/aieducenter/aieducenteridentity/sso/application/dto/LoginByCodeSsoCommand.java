...
/**
 * /api/auth/login-code 验证码登录命令（CONTEXT 登录契约 / issue #22）。
 *
 * <p>identity-web 登录页提交：authorize 上下文（client/redirect/state/nonce/scope）+ 联络方式（account：
 * 邮箱或手机号）+ 验证码（purpose=LOGIN，经 verification 上下文验）。与 /api/auth/login 同一契约，
 ...
 * @param code        验证码（先经 /api/account/verification-code/* 以 LOGIN 用途下发）
 *
 * @since 0.1.0
 */
