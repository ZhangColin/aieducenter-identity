 ...
 * {@link ManagementCallerInterceptor} 比对验签后 {@code ApiKeyInfo.apiKey()}（= app_code）
 * {@code ∈ authorizedCallers}，非白名单 → 403。默认 {@code admin-console}
 * （admin BFF 在 app-registry 的 app_code）。</p>
 *
 * @since 0.1.0
 */
@Component
...
public class ManagementProperties {
    ...
    /**
     * 允许调管理端点的签名调用方白名单（app-registry 的 app_code，即稳定标识 apiKey——非可变显示名）。
    ...
    /** 调用方 app_code（验签后 ApiKeyInfo.apiKey()）是否在白名单内（null / 空白安全——null 不放行）。 */
    public boolean isAuthorized(String callerAppId) {
        if (callerAppId == null || callerAppId.isBlank()) {
            return false;
        }
        return authorizedCallers.contains(callerAppId);
    }
}
